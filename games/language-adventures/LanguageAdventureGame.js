...
import { ASSET_ROOT, NEW_ART_ROOT, SCENARIOS } from './languageData.js';

const clamp = (v,min,max)=>Math.max(min,Math.min(max,v));

export class LanguageAdventureGame extends GameModule {
  static metadata = { id:'language-adventures', name:'🗺️ Little Adventures', description:'Explore, learn, and make kind choices.', version:'2.1.0', author:'Baby Games', assetPath:'games/language-adventures/assets/' };
  ...
  renderStep(){
    ...
    this.view.innerHTML=`<section class="la2-play"><div class="play-top"><button class="back-btn" data-back>← Adventures</button><div class="step-title"><span>${s.icon}</span><strong>${s.title}</strong><div class="dots">${s.steps.map((_,i)=>`<i class="${i<=this.stepIndex?'on':''}"></i>`).join('')}</div><small>${this.stepIndex+1}/${s.steps.length}</small></div><button class="sound-btn" data-speak>🔊</button></div><div class="play-scene tone-${s.tone}" style="--scene:url('${NEW_ART_ROOT}${s.art}')"><div class="scene-wash"></div>${s.id==='home'?`<div class="mumma-stage"><img data-mumma src="${NEW_ART_ROOT}mumma-card.png" alt="Mumma"></div>`:''}<div class="character-stage"><img class="scene-avatar character-idle" data-character src="${NEW_ART_ROOT}characters/toddler/idle.png" alt="Explorer"><div class="character-bubble" data-character-bubble aria-live="polite"></div></div><div class="globe-companion" data-companion>${this.iconFor('globe')}</div><div class="phrase-card"><div class="phrase">${step.phrase}</div><div class="prompt">${step.prompt}</div></div><div class="interaction" data-interaction></div><div class="guide">💡 ${this.guideFor(step)}</div></div></section>`;
  ...
  targetLabel(k){return {child:'Explorer',friend:'Friend',bin:'Recycle',teddy:'Teddy'}[k]||k;}
