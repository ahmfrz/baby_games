...
export class LanguageAdventureGame extends GameModule {
  ...
  setCharacterReaction(state, message=''){ const avatar=this.view?.querySelector('[data-character]'); const companion=this.view?.querySelector('[data-companion]'); const bubble=this.view?.querySelector('[data-character-bubble]'); if(avatar){ avatar.dataset.state=state; avatar.classList.remove('character-idle','character-point','character-happy','character-surprised'); avatar.classList.add(`character-${state}`); } if(companion){ companion.dataset.state=state; } if(bubble){ bubble.textContent=message; bubble.classList.toggle('show',Boolean(message)); } }
  guideFor(step){
    const guides={
