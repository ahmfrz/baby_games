...
export class LanguageAdventureGame extends GameModule {
  ...
  renderStep(){
    ...
    this.view.innerHTML=`<section class="la2-play"><div class="play-top"><button class="back-btn" data-back>← Adventures</button><div class="step-title"><span>${s.icon}</span><strong>${s.title}</strong><div class="dots">${s.steps.map((_,i)=>`<i class="${i<=this.stepIndex?'on':''}"></i>`).join('')}</div><small>${this.stepIndex+1}/${s.steps.length}</small></div><button class="sound-btn" data-speak>🔊</button></div><div class="play-scene tone-${s.tone} ${s.id==='home'?'is-home-scene':''}" style="--scene:url('${s.art.startsWith('scenes/') ? SCENE_ROOT + s.art.slice(7) : NEW_ART_ROOT + s.art}')"><div class="scene-wash"></div>${s.id==='home'?`<div class="mumma-stage"><img data-mumma src="${NEW_ART_ROOT}characters/mumma/kneel.webp" alt="Mumma"></div>`:''}<div class="character-stage"><img class="scene-avatar character-idle" data-character src="${NEW_ART_ROOT}characters/toddler/idle.webp" alt="Explorer"><div class="character-bubble" data-character-bubble aria-live="polite"></div></div>${s.id!=='home'?`<div class="globe-companion" data-companion>${this.iconFor('globe')}</div>`:''}<div class="phrase-card"><div class="phrase">${step.phrase}</div><div class="prompt">${step.prompt}</div></div><div class="interaction" data-interaction></div><div class="guide">💡 ${this.guideFor(step)}</div></div></section>`;
